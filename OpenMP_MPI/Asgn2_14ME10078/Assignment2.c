...
void checkImage(int * image_copy,int * image,const int size){
    ...
    for(int i = 0;i<size;i++){
        for(int j = 0;j<size;j++){
            index = ((long)i)*size + j;
            if(fabs(image_copy[index] - image[index])>1e-5){
...
int main(){
    ...
    #pragma omp parallel for collapse(2) private(sum,mean)     
    for(i = 0;i<max_strides;i++){
        for(j = 0;j<max_strides;j++){
            sum = 0;
            for(int k = 0;k<segment_size;k++){
    ...
    #pragma omp parallel firstprivate(k,l,sum_copy)
    {
        ...
        #pragma omp for collapse(2)
        for(int i = 0;i<size;i++){
            for(int j = 0;j<size;j++){ practical skills taught by deep learning experts including Sebastian Thrun, Ian Goodfellow, Andrew 
                k = i%segment_size;
                l = j%segment_size;
                if(sum_segment[k][l]<20*segment_area){
                    image[i*size + j] = 0;
                }
            }
        }
    ...
    wtime = omp_get_wtime()-wtime;
    printf("\nParallel implementation(2) of Part 2 took %lf s.\n",wtime);
